#!/bin/bash
# auto_test.sh: テストケース作成・実行・CSV出力を自動化

set -e

SRC_DIR="$(dirname "$0")/src"
TEST_FILE="$SRC_DIR/testcase.test.js"
RESULT_CSV="$(dirname "$0")/testresult.csv"
...
# 2. テスト実行（JSON出力）
npx jest "$TEST_FILE" --json --outputFile=testresult.json

# 3. CSV出力
node << EOF
const fs = require('fs');
const result = JSON.parse(fs.readFileSync('testresult.json', 'utf8'));
const csv = ['testcase,status'];
result.testResults[0].assertionResults.forEach(t => {
  csv.push(`${t.title.replace(/,/g, '，')},${t.status}`);
});
fs.writeFileSync('$RESULT_CSV', csv.join('\n'));
EOF

echo "テストケース作成・実行・CSV出力が完了しました。"
